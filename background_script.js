		...
		grabTabSettings(currentTab.windowId, currentTab, function(tabSetting){
			if(tabSetting.reload && !include(settings.noRefreshList, currentTab.url) && currentTab.url.substring(0,19) != "chrome://extensions"){
				chrome.tabs.query({ windowId: timerWindowId }, function(tabs) {
					...
					chrome.tabs.reload(tabs[index].id, {bypassCache:true}, switchToNextTab(timerWindowId, currentTab));
				});
			} else {
...
function badgeTabs(text, windowId) {
	if(text === "default") {
		...
		chrome.tabs.query({"windowId": windowId, "active": true}, function(tab){
			if(text === "on") {
				chrome.browserAction.setIcon({ path: {
					"128": "reporpoised-on-128.png",
					"48": "reporpoised-on-48.png",
					"32": "reporpoised-on-32.png",
					"16": "reporpoised-on-16.png"
				}});
			} else if (text === "pause"){
